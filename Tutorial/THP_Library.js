...
function Osmosis() {
    ...
    this.updateCallback = function(x, y, width, height, scale, rotation, centerX, centerY) {
        logDebug("UPDATED: " + x + ", " + y + ", " + width + ", " + height + ", " + scale + ", " + rotation + ", " + centerX + ", " + centerY);
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;        
    ...
    this.setPosition = function(x, y) {
        ...
        if(typeof this.width !== "number" || typeof this.height !== "number") {
            logError("Node " + getId() + " has invalid height or width");
            return;
        } 
    ...
    this.setDimensions = function(width, height) {
        ...
        if(typeof this.x !== "number" || typeof this.y !== "number") {
            logError("Node " + getId() + " has invalid x or y");
            return;
        } 
    ...
    this.addChild = function(child) {
        ...
        /*
           if(this instanceof Layer && child instanceof Label) {
           logError("Cannot add a Label as a child to a Layer. Use the Scene instead");
           return;
           }
           */

        /*
           if(this instanceof Layer && child instanceof Primitive) {
           logError("Cannot add a Primitive as a child to a Layer. Use the Scene instead");
           return;
           }
           */
        ...
        if (typeof child === "string") {
            ...
            //logDebug("Added " + child.getId() + " as a child to " + this.id);
        }
    };
...
function BaseSprite(url, plugin, x, y, width, height) {
    ...
    this.setShape = function(shape) {
        if(typeof shape !== "string") {
            logError("Invalid shape supplied (not a string)");
            return;
        }
...
function Sprite(plugin, url, x, y, width, height) {
    ...
    this.applyBrightness = function(brightness) {
        // Calculate function
        var func = [(255 * brightness), 1];
        if (!this.filters.brightness) {
            // Create a new filter
            this.filters.brightness = this.plugin.applyFilterFunction(this.id, func, "hsv", false, false, true);
        } else {
            // Update existing filter
            this.plugin.alterFilterFunction(this.id, this.filters.brightness, func);
        }
    };

    this.applyContrast = function(contrast) {
        // Calculate function
        var func = [(contrast * -128) + 128, contrast];
        if (!this.filters.contrast) {
            // Create a new filter
            this.filters.contrast = this.plugin.applyFilterFunction(this.id, func, "hsv", false, false, true);
        } else {
            // Update existing filter
            this.plugin.alterFilterFunction(this.id, this.filters.contrast, func);
        }
    };

    this.applyGaussianBlur = function(size, factor) {
        if (!this.filters.gaussian) {
            // Create a new filter
            this.filters.gaussian = this.plugin.applyGaussianBlur(this.id, size, factor);
        } else {
            // Update existing filter
            this.plugin.alterGaussianBlur(this.id, this.filters.gaussian, size, factor);
        }
    };

    this.removeFilter = function(filter) {
        if (filter != undefined) {
            this.plugin.removeFilter(this.id, filter);
            if (this.filters.gaussian === filter) {
                this.filters.gaussian = undefined;
            } else if (this.filters.brightness === filter) {
                this.filters.brightness = undefined;
            } else if (this.filters.contrast === filter) {
                this.filters.contrast = undefined;
            }
        }
    };
}
...
function Scene(passPlugin, passStep) {
    ...
    this.initQuiz = function() { logDebug("initQuiz() default callback"); };

    // Default loadQuiz callback to be replaced by the developer
    this.loadQuiz = function() { logDebug("loadQuiz() default callback"); };

    // Default checkAnswer callback to be replaced by the developer
    this.checkAnswer = function() { logDebug("checkAnswer() default callback"); return true;};

    // Default resetQuiz callback to be replaced by the developer
    this.resetQuiz = function() { logDebug("resetQuiz() default callback"); };

    // Default showCorrectAnswer callback to be replaced by the developer
    this.showCorrectAnswer = function() { logDebug("showCorrectAnswer() default callback"); };

    // Default cleanUp callback to be replaced by the developer
    this.cleanUp = function() { logDebug("cleanUp() default callback"); };
...
function BaseLabel() {
    ...
    this.setBold = function(bold) {
        if (typeof bold != "boolean") {
            logError("Error: argument not boolean");
            return;
        }
    ...
    this.setAnchor = function(anchor) {
        if (typeof anchor !== "string") {
            logError("Error: Argument to setAnchor a string");
            return;
        }
...
function DropDown(plugin, x, y, width, height) {
    ...
    this.setDefaultOption = function(option) {
        if(typeof option != "string") {
            logError("Attempted to pass " + option + " as string to DropDown " + this.getId());
            return;
        }
...
function Bezier(plugin, points) {
    ...
    this.setPoints = function(points) {
        if(points.constructor.toString().indexOf("Array") != -1) { // Object is an array
            ...
            logError("Attempted to pass " + option + " which is not an Array to Bezier " + this.getId());
        }
    };
    ...
    this.setThickness = function(thickness) {
        if(typeof thickness == "number") {
            ...
            logError("Attempted to pass " + option + " which is not a number to Bezier " + this.getId());
        }
    };
}
...
function THP_Template(plugin, width, height, sceneDescriptor) {
    ...
    this.layoutFrameSprite.setShape("square");
    ...
    this.answerPanelSprite.setShape("square");
    ...
    this.menuBottomSprite.setShape("square");
    ...
    this.demoStatusSprite.setShape("square");
    ...
    this.backToPG.bind("mouse_up", function() { 
            console.log("Calling backToPG: " + that.plugin.loadPhoneGap); 
    ...
    this.progressBarSprite.setShape("square");
    ...
    this.progressLeftSprite.setShape("square");
    ...
    this.progressRightSprite.setShape("square");
    ...
    this.progressMiddleSprite.setShape("square");
    ...
    this.instructionsSprite.setShape("square");
	...
	this.instructionsButton.setShape("square");
	...
	this.Mouse = new Mouser(this.plugin);
    ...
    this.submitAnswer = function() {
        if(this.totalFinished === this.totalScenes) {
        		logError("The demo tried to submit a question after it's done answering all the questions.");
            return;
        }

        // If this scene as been completed, just return
        if(this.getCurrentScene().getCompleted()) {
        		logError("The demo tried to submit a question after the question has been completed.");
            return;
        }
	...
	this.retractAnswerPanel = function() {
        ...
        if(!this.answerPanelLock) {
			...
			//this.titleLabel.addMoveBy(60, 0, this.travelTime);
	...
	this.retractBottomPanel = function() {
        ...
        if(!this.bottomPanelLock) {
            ...
            //this.triesLabel.addMoveBy(0, -50, this.travelTime);
	...
	this.answerMoveDone = function(instance) {
        ...
        // Check if the answer panel is state is retracting
			...
			logError("The answerMoveDone callback was called while the answer panel was already static");
        }
	};
    ...
    this.begin = function() {
        ...
        this.preload_layer.setColor(1.0, 1.0, 1.0, 1.0);

		// Create the background image 
        this.preload_sprite = new Sprite(this.plugin, "pretty_logo_alpha.png", 160, 124, 150, 150);

        // Create the loading label
        this.preload_label = new Label(this.plugin, "Loading...", 2, 200, 75, 100, 30);
        this.preload_label.setColor(1.0, 1.0, 1.0, 1.0);
        ...
        this.preload_scene.addChild(this.preload_loadingbar_container);
        this.preload_scene.addChild(this.preload_loadingbar);
        this.plugin.setScene(this.preload_scene.getId());
        logDebug("setting scene to: " + this.preload_scene.getId()); 
